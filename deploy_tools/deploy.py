...
def deploy(user='lukasz-gajownik',repo=None):
    if not repo:
        print('Run with repo=...')
        return
    domain_folder = os.path.dirname(__file__)
...
def _update_virtualenv(site_folder, virtualenv_folder, domain_name):
    if not os.path.exists(virtualenv_folder):
        _execude_command('cd %s && virtualenv %s -p /usr/local/bin/python3.5') % (virtualenv_folder, )
